...
**Reason**: The pipeline column on each estimate row is removed. Per-stage `PhaseCell` rendering moves into the expanded StageDetail panel only (see `phase-cell-upload` modifications).

**Migration**: Users who previously clicked a per-row PhaseCell SHALL expand the row and click the PhaseCell inside the StageDetail panel, or SHALL use the rollup-level bulk-upload buttons on Month and Week headers.
