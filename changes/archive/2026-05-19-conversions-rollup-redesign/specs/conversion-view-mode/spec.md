...
### Requirement: Mode controls upload cell
**Reason**: The per-row upload cell (PhaseCell rendered in the row's pipeline column) is removed entirely from `PipelineRowItem` (see `phase-cell-upload` modifications). Row-level upload affordance is reachable only through the expanded StageDetail panel and the rollup-level bulk-upload buttons, so mode no longer controls which PhaseCell renders in the row.

**Migration**: The PhaseCell component still exists and continues to render inside the expanded detail panel for retry workflows. Users previously clicking the per-row PhaseCell SHALL expand the row and use the StageDetail panel's upload control, or SHALL use the Month / Week rollup bulk-upload buttons.
