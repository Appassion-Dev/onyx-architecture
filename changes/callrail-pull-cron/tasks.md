...
- [ ] 1.3 Verify behavior locally: POST `{}` with the service-role bearer → 200; POST with no bearer → 401; POST with a junk bearer → 401.
...
- [ ] 3.1 Deploy the updated edge function (`mcp__supabase__deploy_edge_function`).
- [ ] 3.2 Smoke-test: POST `{}` with the service-role bearer; confirm HTTP 200 and that `callrail_leads` gains rows whose `start_time` falls in the last 24 hours.
