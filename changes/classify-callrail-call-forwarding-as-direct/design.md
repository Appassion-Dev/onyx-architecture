...
The existing `%direct%` branch matches the literal `source = 'Direct'` value (34 rows) but not `'Call forwarding'`. Distribution of CallRail `(source, medium)` pairs in production confirms `'Call forwarding'` is the only material miss in the Direct bucket; remaining gaps (`Van Wrap`, `Facebook Ads / paid-social`) are 3 and 1 rows respectively and are out of scope here.

## Goals / Non-Goals

**Goals:**
- Classify CallRail leads with `source = 'Call forwarding'` (40 rows) as `channel = 'Direct'` in `vw_conversion_candidates`.
...
- **[Trade-off] We're patching one source-string value rather than fixing the underlying "classifier reads `source`, not `medium`" structural defect.** Acknowledged. The structural fix (project `cl.medium` into `call_agg`, add a medium-based branch) is the right long-term move but is intentionally scoped out. This change closes 40 of the 43 known miscategorized rows; the remaining 3 (`Van Wrap`) plus future additions can drive the structural change when warranted.
...
1. **Pre-deploy verification.** Confirm the affected-row count matches expectation:
   ...
   ```
   Expected ≈ 40 (matches the production CallRail `(source, medium) = ('Call forwarding', 'direct')` count). A materially different count means assumptions about precedence interaction need re-checking before applying.
