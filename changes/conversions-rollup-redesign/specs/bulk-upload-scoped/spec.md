...
**Reason**: Per-bucket bulk upload is removed. The Conversions page exposes exactly one bulk-upload affordance — a single Upload button in the page header (`ConversionsHeroHeader`) — replacing the per-week and per-month Upload buttons.

**Migration**: Users who previously clicked a Week or Month Upload button SHALL use the top-of-page Upload button, which uploads pending rows across the currently filtered hierarchy. To restrict the scope, narrow the hierarchy via filters (channel, campaign, assignee, mode) before triggering upload.

### Requirement: Bulk upload opens a Dialog confirm modal
**Reason**: Replaced by the new "Bulk upload confirm flow uses Dialog then countdown toast" requirement, which describes the same flow originating from the single page-level button.

**Migration**: The Dialog confirm + countdown toast pattern is retained, just with one origin point instead of three (page header instead of page header + every week header + every month header).

### Requirement: Confirming bulk upload shows a countdown toast
**Reason**: Replaced by the new "Bulk upload confirm flow uses Dialog then countdown toast" requirement, which subsumes the same behavior.

**Migration**: No user-facing change to the countdown toast itself.

### Requirement: Bulk upload scope is limited to the time bucket's estimate IDs
**Reason**: Replaced by the new "Bulk upload scope is limited to the explicit estimate ID list" requirement, which generalizes from "time bucket's IDs" to "filtered visible hierarchy's IDs" — since there is no per-bucket button anymore.

**Migration**: To upload only a specific week or month worth of rows, use the existing date-range hierarchy along with filters to narrow the visible set, then trigger the single page-level Upload button.
