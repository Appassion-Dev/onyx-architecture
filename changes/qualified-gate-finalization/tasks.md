...
- [ ] 4.4 After the truncate migration, confirm `work_timestamps` is empty; after a test import, confirm exactly one row per estimate/job (no `Date.now()` duplicates) keyed `ts_est_<id>` / `ts_job_<id>`. _(Pre-state + truncate safety + importer determinism verified; the empty-after-truncate and one-row-per-entity-after-import checks require deployment — see task 4.5.)_
