# gads-upload-payload-slices

## Purpose

Expose per-estimate Google Ads conversion upload payload slices (request, response results, and errors) derived from each upload row's batch payload, scoped to the opened estimate and narrowed to a single conversion type, for display in the estimate conversion detail panel.
