# gads-upload-step-tests Specification

## Purpose

Per-module test coverage requirements for the Google Ads conversion upload edge function. This capability defines which exported functions must have unit tests, what branches each test must exercise, the shared fake-Supabase pattern (`test-helpers.ts`), and the orchestrator-level integration test that uses the `_mock_response` request-body hook to prove `handlePost` routes correctly end-to-end without invoking real HTTP or env-backed Supabase. The coverage is strictly additive on the test surface; production behavior does not change.
